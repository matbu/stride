...
void main() {
  ...
  group('éditeur de séance', () {
    ...
    testWidgets('la saisie rapide suggère un exemple adapté à chaque bloc, sans rien pré-remplir', (tester) async {
      tallScreen(tester);
      final fake = FakeSessions();
      await openEditor(tester, newSessionDraft(date: DateTime(2026, 9, 28)), clubOverrides(sessionActions: fake));

      expect(find.text("Ajouter : Footing 20'"), findsOneWidget, reason: 'échauffement');
      expect(find.text('Ajouter : Étirements'), findsOneWidget, reason: 'retour au calme');
      ...
      // Juste une suggestion, rien de réellement ajouté nulle part : rien à effacer.
      final d = fake.saved.single;
      expect(d.blocks, isEmpty, reason: 'aucun bloc touché, donc aucun enregistré');
    });
